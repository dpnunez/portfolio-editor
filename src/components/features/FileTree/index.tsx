...
function Folder({
  ...
  const statusComponent = status ? (
    <FileStatus status={status as fileStatus} type="dot" />
  ) : (
    sideComponent
  );
...
function File({
  ...
  const statusComponent = status ? (
    <FileStatus status={status as fileStatus} type="label" />
  ) : (
    sideComponent
  );
  ...
  return <MotionHighlightItem value={name}>{content}</MotionHighlightItem>;
}

function FileTree({ files, defaultOpen, open, onOpenChange }: FileTreeProps) {
  return (
    <Accordion
      type="multiple"
      defaultValue={defaultOpen}
...
function FileTreeTopLevel(props: FileTreeProps) {
  return (
    <div
      data-slot="files"
      className={cn(
